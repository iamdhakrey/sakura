...
class Welcome(commands.Cog):
    ...
    @commands.command(name='get_welcome',
    ...
    async def get_welcome(self, ctx: Context):
        ...
        embed.add_field(name="is_enable",
                        value=welcome.welcome_enable,
                        inline=False)
        embed.add_field(name="welcome_channel",
                        value=welcome_channel,
                        inline=True)
        embed.add_field(name="self_role",
                        value=welcome.self_role,
                        inline=False)
        await ctx.send(embed=embed)

    @commands.command(name='Enable Welcome Message',
    ...
    async def test_welcome(self, ctx):
        ...
        if welcome_data.self_role is not None:
            role = discord.utils.get(ctx.guild.roles,
                                     id=int(welcome_data.self_role))
        else:
            role = None
        # welcome_channel = discord.utils.get(ctx.guild.channels,
        #                                      id=int(
        #                                        welcome_data.welcome_channel))

        temp_welcome_msg = norm_to_emoji(ctx.guild,
                                         welcome_data.welcome_msg).split("\n")
        welcome_msg = []

        for msg in temp_welcome_msg:
            wlmsg = msg
            if "member.mention" in msg:
                wlmsg = str(msg).replace("{" + "member.mention" + "}",
                                         member_tag)
                # test.append(wlmsg)
            if "member.name" in msg:
                wlmsg = str(msg).replace("{" + "member.name" + "}",
                                         member_name)
                # test.append(wlmsg)
            if "member.count" in msg:
                wlmsg = str(msg).replace("{" + "member.count" + "}",
                                         str(ctx.guild.member_count))
                # test.append(wlmsg)
            if "member.server_name" in msg:
                wlmsg = str(msg).replace("{" + "member.server_name" + "}",
                                         str(ctx.guild.name))
                # test.append(wlmsg)
            if role is not None:
                if "member.role" in msg:
                    wlmsg = str(msg).replace("{" + "member.role" + "}",
                                             str(role.mention))
                # test.append(wlmsg)
            welcome_msg.append(wlmsg)
        embed = discord.Embed(description="".join(welcome_msg))
        save_image(self.bot, ctx.guild, ctx.author)
        file = discord.File(open(str(ctx.guild.id) + "_out_welcome.png", 'rb'))
        embed.set_image(url="attachment://" + str(ctx.guild.id) +
                        "_out_welcome.png")
        await ctx.send(embed=embed, file=file)

    @commands.command(name='set_welcome_image',
        ...
        for link in _image_list:
            ...
            if width <= 1920 and height <= 972:
                ...
                output = ImageOps.fit(background,
                                      background.size,
                                      centering=(0.5, 0.5))
                output.save("media/images/" + str(ctx.guild.id) + "/" +
                            str(ctx.guild.id) + "_" + str(i) + ".jpg")
                image_name.append(str(ctx.guild.id) + "_" + str(i) + ".jpg")
                if i == 1:
                    await DbConnection.fetch_welcome(
                        ctx.guild,
                        image1="images/" + str(ctx.guild.id) + "/" +
                        str(ctx.guild.id) + "_" + str(i) + ".jpg")
                elif i == 2:
                    await DbConnection.fetch_welcome(
                        ctx.guild,
                        image2="images/" + str(ctx.guild.id) + "/" +
                        str(ctx.guild.id) + "_" + str(i) + ".jpg")
                elif i == 3:
                    await DbConnection.fetch_welcome(
                        ctx.guild,
                        image3="images/" + str(ctx.guild.id) + "/" +
                        str(ctx.guild.id) + "_" + str(i) + ".jpg")
                elif i == 4:
                    await DbConnection.fetch_welcome(
                        ctx.guild,
                        image4="images/" + str(ctx.guild.id) + "/" +
                        str(ctx.guild.id) + "_" + str(i) + ".jpg")
                elif i == 5:
                    await DbConnection.fetch_welcome(
                        ctx.guild,
                        image5="images/" + str(ctx.guild.id) + "/" +
                        str(ctx.guild.id) + "_" + str(i) + ".jpg")
                else:
                    pass
    ...
    async def set_welcome_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            await ctx.reply("Image link require")

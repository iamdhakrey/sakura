...
import requests
from sakura.settings import CLIENT_ID, CLIENT_SECRET, DISCORD_REDIRECT_URL
...
class Discord_API:
    ...
    def get_guild_list(self, access_token) -> List[Dict[str, str]]:
        response = requests.get(
        ...
        for details in response.json():
            ...
            if details.get("owner"):
                ...
                _guilds["icon"] = details.get("icon")
                _guilds["owner"] = details.get("owner")
                if DbConnection.get_sync_server(server_id=details['id']):
                    _guilds["is_added"] = True
                else:

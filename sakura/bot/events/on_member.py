...
def event_setup(bot: Bot):
    ...
    async def on_member_join(member: discord.Member):
        ...
        if welcome_data.welcome_enable is None:
            return
        if welcome_data.self_role is None:
            return

        if welcome_data.welcome_channel:
            w_channel = discord.utils.get(member.guild.channels,
                                          id=int(welcome_data.welcome_channel))
        ...
        temp_welcome_msg = norm_to_emoji(member.guild,
        ...
        file = discord.File(
            open(str(member.guild.id) + "_out_welcome.png", 'rb'))
        embed.set_image(url="attachment://" + str(member.guild.id) +
                        "_out_welcome.png")

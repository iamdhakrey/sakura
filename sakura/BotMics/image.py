...
def save_image(bot,ctx):
    is_w_images = False
    count_list = []
    for i in range(5):
        if os.path.isfile(os.path.join(ROOT_PATH,"images/backgrounds/"+str(ctx.guild.id)+'/'+str(ctx.guild.id)+"_"+str(i)+".jpg")):
            is_w_images = True
            count_list.append(i)
        else:
            pass
    ...
    # messeges
    hello = "Hey Buddy,"
    username = str(bot.get_user(ctx.author.id))
    ...
    output.save(str(ROOT_PATH)+'/images/'+str(ctx.guild.id)+'/avatar.png')
    ava = Image.open(str(ROOT_PATH)+'/images/'+str(ctx.guild.id)+'/avatar.png')
    ava_draw = ImageDraw.Draw(ava)
    ava_draw.arc((0, 0, 380, 380), start=0, end=360, fill=(194,83,111),width=12)
    ava.save(str(ROOT_PATH)+'/images/'+str(ctx.guild.id)+'/avatar.png')
    if is_w_images:
        welc = Image.open(ROOT_PATH+'/images/backgrounds/'+str(ctx.guild.id)+"/"+str(ctx.guild.id)+"_"+str(image_num)+'.jpg' )
    else:
    ...
    welc.save(str(ctx.guild.id)+'_out_welcome.png', format='PNG')
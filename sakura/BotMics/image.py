...
def save_image(bot, guild, member):
    ...
    for i in range(5):
        if os.path.isfile(
                os.path.join(
                    ROOT_PATH, "images/backgrounds/" + str(guild.id) + '/' +
                    str(guild.id) + "_" + str(i) + ".jpg")):
            is_w_images = True
            count_list.append(i)
        else:
            pass
    ...
    ava = Image.open(
        str(ROOT_PATH) + '/images/' + str(guild.id) + '/avatar.png')
    ...
    if is_w_images:
        welc = Image.open(ROOT_PATH + '/images/backgrounds/' + str(guild.id) +
                          "/" + str(guild.id) + "_" + str(image_num) + '.jpg')
    else:
        welc = Image.open(ROOT_PATH + '/default/welcome' + str(image_num) +
                          '.jpg')
